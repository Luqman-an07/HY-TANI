...
import { 
  ...
  ChevronLeft, ChevronRight, UploadCloud // <--- 1. Import UploadCloud
} from 'lucide-react';
...
// --- 2. Update Props: Tambah offlineQueueCount & onSync ---
const MainLayout = ({ children, user, activeTab, setActiveTab, onLogout, isOffline, toggleOffline, onGlobalScan, offlineQueueCount, onSync }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-800">
      ...
      {/* SIDEBAR */}
      <aside className={`fixed inset-y-0 left-0 z-[50] bg-emerald-900 text-white flex flex-col shadow-2xl transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0 w-72' : '-translate-x-full'} md:relative md:translate-x-0 md:inset-auto md:shadow-none ${isSidebarOpen ? 'md:w-72' : 'md:w-20'}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
        ...
        {/* HEADER */}
        <header className="bg-white/90 backdrop-blur-md h-16 border-b border-slate-200 sticky top-0 z-30 px-4 md:px-6 shadow-sm transition-all duration-300 shrink-0">
          <div className="flex items-center justify-between h-full max-w-7xl mx-auto">
            ...
            <div className={`flex items-center ${showMobileSearch ? 'w-full' : ''} justify-end space-x-2 md:space-x-4`}>
              ...
              <div className={`${showMobileSearch ? 'hidden' : 'flex'} items-center space-x-3`}>
                ...
                {/* --- 3. INDIKATOR ANTRIAN OFFLINE --- */}
                {offlineQueueCount > 0 && (
                    <button 
                        onClick={onSync} 
                        disabled={isOffline} 
                        title={isOffline ? "Menunggu koneksi internet..." : "Klik untuk Sinkronisasi"}
                        className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold border shadow-sm transition-all active:scale-95 ${isOffline ? 'bg-amber-100 text-amber-700 border-amber-200 cursor-help' : 'bg-blue-100 text-blue-700 border-blue-200 hover:bg-blue-200 cursor-pointer animate-pulse'}`}
                    >
                        <UploadCloud size={14} />
                ...
                <div className={`flex items-center space-x-1.5 px-3 py-1 rounded-full text-[10px] font-bold border shadow-sm cursor-help ${isOffline ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`} title={isOffline ? "Data Lokal" : "Terhubung Cloud"}>
