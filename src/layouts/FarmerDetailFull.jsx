...
import { 
  ...
  Clock // <--- PERBAIKAN: Ikon Clock ditambahkan di sini
} from 'lucide-react';
...
// Helper: Format Tanggal & Jam (Untuk Update Satelit)
const formatDateTime = (timestamp) => {
    if (!timestamp) return "Data Manual";
    // Timestamp Agromonitoring dalam detik (Unix), JS butuh milidetik
...
const FarmerDetailFull = ({ farm, onBack, onCompleteVisit, onEdit, onDelete }) => {
  ...
  // Penentuan Fase Tanam yang Lebih Detail
  ...
  const getRecommendations = (status) => {
    switch(status) {
      case 'sehat':
        return { title: 'Pertahankan Kondisi', actions: ["Jaga ketinggian air irigasi macak-macak.", "Lanjutkan pemupukan berimbang sesuai jadwal.", "Amati populasi musuh alami hama."] };
      case 'waspada':
        return { title: 'Perlu Tindakan Preventif', actions: ["Cek apakah ada genangan air berlebih.", "Tambahkan pupuk daun untuk boost nutrisi.", "Inspeksi manual: cari tanda bercak daun."] };
      case 'bahaya':
        return { title: 'INTERVENSI SEGERA', actions: ["Isolasi area yang terkena (jangan alirkan airnya ke petak lain).", "Aplikasi pestisida/fungisida dosis kuratif.", "Konsultasi foto tanaman ke ahli."] };
      default: return { title: 'Data Belum Cukup', actions: ["Lakukan kunjungan lapangan segera."] };
    }
  };
  const rec = getRecommendations(farm.status);

  // --- 4. INTEGRASI WHATSAPP LENGKAP ---
          ...
          <div className="fixed inset-0 z-[100] bg-black/90 flex flex-col animate-in fade-in duration-200">
              <div className="flex justify-between items-center p-4 bg-slate-900 text-white">
                  <div>
                    ...
                    <p className="text-xs text-slate-400">Analisis Citra Satelit</p>
                  </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800 flex items-center">
              {farm.farmer}
              <span className={`ml-3 px-2.5 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wide ${statusColor.badge}`}>
                {farm.status}
              </span>
            </h1>
            <p className="text-xs text-slate-500 flex items-center mt-0.5">
              <MapPin size={12} className="mr-1"/> {farm.name} • {farm.size} Ha
            </p>
          </div>
        </div>
        ...
        <div className="hidden md:flex items-center space-x-4 mr-4 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
            ...
            <div>
                ...
                <div className="flex items-center text-[10px] text-slate-500 space-x-2">
                    <span>32°C</span>
                    <span className="flex items-center"><Wind size={10} className="mr-1"/> 12 km/h</span>
                    <span className="flex items-center"><Droplets size={10} className="mr-1"/> 60%</span>
                </div>
            </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <div className="relative pt-2 pb-6">
                    ...
                    {/* Markers (0, 30, 60, 90 HST) */}
                    <div className="absolute top-0 bottom-0 left-[0%] w-px h-full">
                        <div className="absolute top-6 left-0 -translate-x-1/2 text-[9px] font-bold text-slate-400">Tanam</div>
                    </div>
                    <div className="absolute top-0 bottom-0 left-[27%] w-px bg-slate-200 h-full">
                        <div className="absolute top-6 left-0 -translate-x-1/2 text-[9px] text-slate-400">30 HST</div>
                    </div>
                    <div className="absolute top-0 bottom-0 left-[54%] w-px bg-slate-200 h-full">
                        <div className="absolute top-6 left-0 -translate-x-1/2 text-[9px] text-slate-400">60 HST</div>
                    </div>
                    <div className="absolute top-0 bottom-0 left-[81%] w-px bg-slate-200 h-full">
                        <div className="absolute top-6 left-0 -translate-x-1/2 text-[9px] text-slate-400">90 HST</div>
                    </div>
                </div>
            </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col md:flex-row h-[500px] md:h-[400px]">
                        ...
                        <div className="w-full md:w-1/2 h-1/2 md:h-full relative bg-slate-100 border-b md:border-b-0 md:border-r border-slate-200 group">
                            ...
                            <MapContainer center={[centerLat, centerLng]} zoom={15} scrollWheelZoom={true} zoomControl={false} style={{ height: "100%", width: "100%" }}>
                                ...
                                <Polygon positions={farmPolygon} pathOptions={{ color: statusColor.border, fillColor: statusColor.fill, fillOpacity: 0.5, weight: 2 }}>
                                    <Tooltip sticky direction="top" offset={[0, -10]}>
                                        <div className="text-xs font-bold">{farm.name}</div>
                                        <div className={`text-[10px] uppercase ${statusColor.text}`}>{farm.status}</div>
                                    </Tooltip>
                                </Polygon>
                                <Marker position={[centerLat, centerLng]}></Marker>
                                <FitBoundsToPolygon polygon={farmPolygon} />
                            ...
                            <div className="absolute bottom-3 left-3 z-[400] bg-white/90 backdrop-blur px-3 py-1.5 rounded-lg shadow border border-slate-200 text-xs font-medium text-slate-600">
                                Visualisasi Bentuk & Kesehatan Lahan
                            </div>
                        </div>
                        ...
                        <div className="w-full md:w-1/2 p-6 flex flex-col h-1/2 md:h-full overflow-y-auto">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-sm font-bold text-slate-700 flex items-center">
                                    <Leaf size={16} className={`mr-2 ${statusColor.text}`}/> Analisis Satelit
                                </h3>
                                {/* TIMESTAMP UPDATE REAL-TIME */}
                                <div className="flex flex-col items-end">
                                    <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Terakhir Update</span>
                                    <span className="text-xs font-mono font-bold text-slate-600 flex items-center bg-slate-100 px-2 py-1 rounded mt-1">
                                        <Clock size={10} className="mr-1.5"/> 
                                        {formatDateTime(farm.satelliteDate)}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className={`p-4 rounded-xl border mb-4 ${statusColor.bg} border-opacity-60`}>
                                <div className="flex items-center mb-2">
                                    <Lightbulb size={18} className={`mr-2 ${statusColor.text}`} />
                                    <h4 className={`font-bold text-sm ${statusColor.text}`}>{rec.title}</h4>
                                </div>
                                <p className="text-xs text-slate-600 leading-relaxed mb-0">
                                    Citra satelit menunjukkan indeks kesehatan vegetasi <b>{farm.ndvi}</b>. Pola warna pada peta merepresentasikan sebaran klorofil tanaman.
                                </p>
                            </div>
                                ...
                                <ul className="space-y-3">
                                    {rec.actions.map((a, i) => (
                                        <li key={i} className="flex items-start text-sm text-slate-700">
                                            <CheckCircle size={16} className="mt-0.5 mr-3 text-emerald-500 shrink-0"/>
                            ...
                            <button onClick={handleWhatsApp} className="mt-6 flex items-center justify-center space-x-2 bg-emerald-600 text-white px-4 py-3 rounded-xl shadow-lg shadow-emerald-200 hover:bg-emerald-700 transition-all active:scale-95 w-full">
                                <MessageCircle size={18}/>
                                <span className="font-bold text-sm">Kirim Instruksi ke Petani</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                        ...
                        <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                            <div>
                                <p className="font-bold text-slate-800 text-sm">{farm.farmer}</p>
                                <p className="text-xs text-slate-500 font-mono mt-0.5">{contactNumber}</p>
                            </div>
                            <button onClick={handleWhatsApp} className="p-2.5 bg-emerald-100 text-emerald-600 rounded-full hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                                <Phone size={18}/>
                            </button>
                        </div>
                    </div>
                        ...
                        <div className="relative border-l-2 border-slate-100 ml-2.5 space-y-8">
                            {historyLog.length === 0 ? (
                                ...
                                <div key={idx} className="relative pl-6">
                                    ...
                                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 text-xs text-slate-600 leading-relaxed shadow-sm">
                                        {log.note || log.notes || 'Pengecekan rutin dan wawancara petani.'}
                                    </div>
                                </div>
                            ))}
                        </div>
